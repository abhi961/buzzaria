...
const Latestscreen = ({ navigation }) => {
  ...
  const fetchProduct = async () => {
    try {
      const response = await axios.get(
        "https://trijatta.tech/Buzzaria_api/Products/product/0/latest"
      );
      dispatch({ type:"GET_LATEST_PRO", payload:response.data });
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const openProduct = (product_id) => {
    console.log(product_id, "********** productId Latest product");
